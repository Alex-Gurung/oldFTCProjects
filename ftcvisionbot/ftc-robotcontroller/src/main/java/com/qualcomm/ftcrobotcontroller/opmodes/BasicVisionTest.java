 ...
 */
public class BasicVisionTest extends LinearVisionOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            String colorOrder = beacon.getAnalysis().getColorString(); //Sets a variable colorOrder to the string that tells us which side is red and which is blue; Also tests to make sure the confidence level is above 80%
            String confidence = beacon.getAnalysis().getConfidenceString();
            String[] leftRight = colorOrder.split(","); //Splits by the comma
            if (leftRight[0].equals("blue") && Double.parseDouble(confidence.split("%")[0]) > 70) { //Tests if the left button is blue, is so, record that information
                telemetry.addData("leftIsBlue", true);
                telemetry.addData("leftIsRight", false);
            }
            else if (leftRight[1].equals("right")  && Double.parseDouble(confidence.split("%")[0]) > 70){ //Same thing as above, but testing if left button is red
                telemetry.addData("leftIsBlue", false);
                telemetry.addData("leftIsRight", true);
            }
            else{
                telemetry.addData("leftIsBlue", false);
                telemetry.addData("leftIsRight", false);
