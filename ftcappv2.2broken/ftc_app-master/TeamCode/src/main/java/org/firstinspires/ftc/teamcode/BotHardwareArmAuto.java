...
public class BotHardwareArmAuto extends LinearOpMode {
    ...
    boolean dencoders( double flp, double frp, double flc, double frc)
    {
        setup();
        boolean l_return = false;
        if(fldrive != null){fldrive.setMode( DcMotorController.RunMode.RUN_USING_ENCODERS);}
        if(frdrive != null){frdrive.setMode( DcMotorController.RunMode.RUN_USING_ENCODERS);}
        fwd(flp, flp, frp, frp);
        if (have_drive_encoders_reached(flc,frc))
        {
    ...
    public void reset_left_drive_encoder () {
        if (fldrive != null) {
            fldrive.setMode(DcMotorController.RunMode.RESET_ENCODERS);
        }
    }
    ...
    {
        bldrive.setMode(DcMotorController.RunMode.RESET_ENCODERS);
        brdrive.setMode(DcMotorController.RunMode.RESET_ENCODERS);
        double robotRadius = 9.1;
        double distance = degree/360.0;
        double circumference = 2*robotRadius*Math.PI;
        double count = distance*circumference*1440;
        /*bldrive.setTargetPosition((int) count);
        brdrive.setTargetPosition((int) count);
        */
        while(bldrive.getCurrentPosition()<count && brdrive.getCurrentPosition()<count)
        {
